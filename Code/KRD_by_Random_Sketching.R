here::i_am("Code/KRD_by_Random_Sketching.R")
require(here)
require(CVXR)
...
sketching_matrix <- matrix(rnorm(k*n), nrow=k, ncol=n)
S <- sketching_matrix

# USE 3rd equation from paper
# So we'll solve this argmin using CVXR
c = CVXR::Variable(k)
objective = {-sum((R %*% t(S)) %*% c)/n} +
  { {(phi%*%t(S)) %*% c} %>% exp() %>% mean()} + 
  { lambda * quad_form(t(S)%*%c, R)}
